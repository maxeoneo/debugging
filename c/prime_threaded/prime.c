...
int nthreads,       // number of threads (not counting main())
    n,              // upper bound of range in which to fine primes
    prime[MAX_N+1], // in the end, prime[i] = 1 if pime, else 0
...
void worker(int tn) // tn is the thread number (0, 1, ...)
{
  ...
  do {
    // get next sieve multiplier, avoid duplication across threads
    pthread_mutex_lock(&nextbaselock);
    base = nextbase += 2;
    pthread_mutex_unlock(&nextbaselock);
    if (base <= lim) {
      work[tn]++; // log work done by this thread
      // don't bother with corssing out if base is know to be composite
      if (prime[base])
        crossout(base);
    }
    else return;
